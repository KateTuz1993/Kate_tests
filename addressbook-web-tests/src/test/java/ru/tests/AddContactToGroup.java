...
public class AddContactToGroup extends TestBase{
    ...
    public void testContactDeletion(){
        ...
        GroupData selectedGroup = allGroups.iterator().next(); //выбираем первую попавшуюся группу
        ...
        if (linkedGroups.equals(allGroups)){ //если контакт уже входит во все группы, то создаем новую группу
            ...
            Iterator<GroupData> iterator = allGroups.iterator();
            while (iterator.hasNext()){ //перебираем все группы
                if (linkedGroups.contains(allGroups.iterator().next())) {continue;}
                else {
                    //добавление контакта в группу
                    app.goTo().goToHomePage();
                    app.contact().addToGroup(modifyingContact, selectedGroup);
                    break;
                }
            }
