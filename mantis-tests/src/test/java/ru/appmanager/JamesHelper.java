...
public class JamesHelper {
    ...
    private void initTelnetSession(){
        ...
        try {
            telnet.connect(mailserver, port); //устанавливаем соединение с почтовыым сервером
            in = telnet.getInputStream(); //берем входной поток для чтения данных
            out = new PrintStream(telnet.getOutputStream()); //берем выходной поток для того, чтобы отправлять команды
        } catch (Exception e){
            e.printStackTrace();
        }
        readUntil("Login id:"); //ждем пока не появится надпись
        write("");
        readUntil("Password:");
        write("");
